...
function HomePage({match, history}) {
    ...
    return (
        <div className='home-page'>
            <Meta title="bookstorelk | Buy books online" />
            <HomeMain />
            <Container maxWidth={'lg'}>
                <GenreSelector />
                <TopRated>
                    <h1 ref={headingRef}>Top Rated Books</h1>
                    {loading ?  <div style={{height:gridRef.current.offsetHeight + 60, width:gridRef.current.offsetWidth, display: 'grid', placeItems: 'center'}}>
                        <Loader/>
                    </div> : 
